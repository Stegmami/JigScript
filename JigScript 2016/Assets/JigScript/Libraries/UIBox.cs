...
{
    public class UIBox : UIControlsBase
    {
        Value v;
        GUIStyle style;
        ...
        public UIBox(UIControlsBase.Location L, Rect rect, Value v, bool active) : base(L, rect, active)
        {
            this.v = v;
        }
        ...
        public void Update()
        {
            if (active)
            {
                ...
                if ( style != null )
                {
                    GUI.Box(GetRect(), GetContent(v), style);
                }
                else
                {
                    GUI.Box(GetRect(), GetContent(v));
                }
            }
        }               
    };
